...
function Estrellas({ rating }) {
  return (
    <span style={{ color: '#F97316', fontSize: '13px' }}>
      ...
      <span style={{ color: '#6B7280', marginLeft: '4px' }}>{rating} ({Math.floor(Math.random() * 200 + 20)})</span>
    </span>
  )
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FA' }}>
      ...
      <nav style={{ background: '#1B3A6B', padding: '0 48px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ color: '#fff', fontSize: '20px', fontWeight: '600', letterSpacing: '-0.5px' }}>
        ...
        <div style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
          ...
          <a href="#" style={{ color: '#93C5FD', fontSize: '14px', textDecoration: 'none' }}>Cómo funciona</a>
          <button style={{ background: 'transparent', border: '1px solid #93C5FD', color: '#93C5FD', borderRadius: '8px', padding: '8px 16px', fontSize: '14px', cursor: 'pointer' }}>Iniciar sesión</button>
      ...
      <div style={{ background: '#EEF2FF', padding: '64px 48px 48px', borderBottom: '1px solid #E5E7EB' }}>
        <div style={{ maxWidth: '680px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#fff', border: '1px solid #E5E7EB', borderRadius: '999px', padding: '6px 14px', fontSize: '13px', color: '#1B3A6B', marginBottom: '24px' }}>
            <span style={{ color: '#F97316' }}>✓</span> Más de 2.400 profesionales verificados en Chile
          </div>
          ...
          <div style={{ display: 'flex', gap: '0', background: '#fff', border: '1px solid #E5E7EB', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', padding: '0 20px', gap: '10px', borderRight: '1px solid #E5E7EB' }}>
              ...
              <input placeholder="¿Qué servicio necesitas?" style={{ border: 'none', outline: 'none', fontSize: '15px', color: '#111827', width: '100%', padding: '18px 0', background: 'transparent' }} />
            </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 48px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div style={{ display: 'flex', gap: '8px' }}>
            {oficios.map((o, i) => (
              <button key={o} style={{
                padding: '8px 18px', borderRadius: '999px', fontSize: '14px', cursor: 'pointer', fontWeight: i === 0 ? '500' : '400',
                background: i === 0 ? '#1B3A6B' : '#fff',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px', marginBottom: '48px' }}>
          {contratistas.map(c => (
            <div key={c.id} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: '16px', padding: '24px', cursor: 'pointer', transition: 'box-shadow 0.2s' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                ...
                {c.verificado && (
                  <span style={{ background: '#ECFDF5', color: '#059669', fontSize: '12px', padding: '4px 10px', borderRadius: '999px', fontWeight: '500' }}>
                    ✓ Verificado
                  </span>
                )}
              </div>
              <p style={{ fontWeight: '600', fontSize: '16px', color: '#111827', marginBottom: '4px' }}>{c.nombre}</p>
              <p style={{ fontSize: '14px', color: '#6B7280', marginBottom: '12px' }}>{c.oficio} · {c.exp} años exp.</p>
        ...
        <div style={{ background: '#1B3A6B', borderRadius: '16px', padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <button style={{ background: '#F97316', border: 'none', color: '#fff', padding: '16px 32px', borderRadius: '10px', fontSize: '16px', fontWeight: '600', cursor: 'pointer', marginBottom: '8px', display: 'block' }}>
              Comenzar gratis →
            </button>
            <span style={{ fontSize: '13px', color: '#93C5FD' }}>No se requiere tarjeta de crédito</span>
          </div>
        </div>
      </div>
    </div>
